## 문자 인코딩의 역사 
1. [[컴퓨터와 문자 인코딩]]
	- ASCII란?
	- ISO_8859_1 : 서유럽 중심 문자 집합
	- EUC-KR : 한국에서도 사용할 수 있는 문자 집합
	- MS949 : 마이크로소프트가 만든 EUC-KR 확장 인코딩
	- 유니코드의 등장 
	- UTF-16 : 다국어 표현 가능. ASCII와 호환 안 됨(2byte 고정)
	- UTF-8 : 다국어 표현 가능. ASCII와 호환 가능. 가변문자코딩
2. [[Charset]]의 메서드
	- forName(obj)
	- StandardCharsets
	- defaultCharset()
	-  예제1) Charset별 실제 바이트가 어떻게 나오는지 확인
	-  예제2) Charset별 인코딩, 디코딩시 호환 여부
## I/O 기본 스트림
1. FileInputStream과 FileOutputStream 사용법 : [[파일 입출력]]
	- 기능
	- 부분으로 나누어 읽기 VS 전체 읽기
2. I/O의 기본 추상 클래스 : [[IO 기본 추상 클래스]]
	- 메모리 스트림 예시
	- 콘솔 스트림 예시
3.  [[추상화의 장점]]
	- 일관성
	- 확장성
	- 재사용성
	- 에러처리
4.  [[IO성능 개선]]
	- 일반적인 FileXxxStream 사용시 속도가 느린 이유
	- 직접 버퍼 사용
	- BufferedOutputStream 사용
	- [[기본 스트림, 보조 스트림]]
	- BufferedInputStream 사용 
	- 버퍼를 직접 다루는 것 보다 BufferedXxx성능이 떨어지는 이유
	- 한 번에 읽고 쓰기
## 문자를 다루는 I/O클래스
1. [[스트림을 문자로]]
	- 문자 -> byte[] 전환해주는 `OutputStreamWriter`
	- byte를 다루는 클래스와 문자를 다루는 클래스 UML
	- `OutputStreamWriter` 마저 간소화한  `FileWriter`
	- `Writer`, `Reader` 클래스의 Buffer 클래스 사용법
## 기타 스트림들
1. [[PrintStream(fos) - 콘솔 출력하듯]]
2. [[DataOutputStream(fos)]] : 데이터 형식 그대로 저장
3. ![[Pasted image 20250528222133.png]]
4.  [[DataOutputStream(fos)]]
	- DataStream 문자를 읽어오는 방식
	- DataStream의 장점 : 최적화
## 객체를 한 번에 파일로 저장하는 방법
1.  [[직렬화 - ObjectXxxputStream]]
	- `Serializable` :  마커 인터페이스
	- ObjectOutputStream() 사용법
	- 직렬화의 한계
2. [[List]].of(obj) : 불변 객체
3. ObjectStream의 대안
	- [[직렬화 - XML]]
	- [[직렬화 - JSON]]
	- [[직렬화 - ProtoBuf, Avro]]
	-  **직렬화 정리**
		- 자바 객체 직렬화는 대부분 사용하지 않는다.
		- JSON이 사실상 표준이다. JSON을 먼저 고려하자.
		- 성능 최적화가 매우 중요하다면 Protobuf, Avro 같은 기술을 고려하자. (대부분 JSON만 사용해도 충분하다)
## File, Files
1. File 기능 정리 : [[File 기능]]
2. Files 특징, 기능 : [[Files 기능]]
3. File과 Files 경로 다루기 : [[경로]]
4. [[Files로 문자 파일 읽기]]
5. [[파일 복사 최적화]]
6. 파일을 다루어야 할 일이 있다면 항상 `Files`의 기능을 먼저 찾아보자.
## 네트워크 기본 이론
1. [[IP]]
	- IP의 역할 과 한계
2. [[TCP]]
	- TCP/IP패킷 정보
	- IP의 한계를 해결한 TCP특징
3. [[UDP(사용자 데이터그램 프로토콜)]]
4. [[PORT]]
5. [[DNS조회 과정]]
## 클라이언트 서버 연결
1. [[클라이언트 - 서버 소켓 연결 과정]]
2. [[클라이언트 - 서버 소켓 연결 과정 (멀티스레드)]]
	- 문제 상황과 해결 방법
	- 클라이언트 2개 소켓 - 서버 1개 소켓 연결 과정  (1개 소켓 연결 안 됨)
	- 프로토콜 단위로 데이터 송신 과정
##  자원정리
1. null체크 할 것
2. [[try-with-resource]]
	- jdk1.7부터 사용 가능
	- 구현 방법 `AutoClosable`
	- try-finally를 사용했을 때 자원 정리 문제점
	- try-with-resources의 장점
	- 부가예외
	- 서버 강제 종료시 try-finally 사용 이유
3. [[셧다운 훅(Shutdown Hook)]]
	- 정상종료의 종류
	- 강제종료의 종류
	- 구현방법
	- 셧다운 훅으로 자원 정리시 `sleep(1000)`하는 이유
4. [[네트워크 연결 예외]]
	- UnknownHostException : 연결 자체 안 됨.
	- ConnectException : 해당IP로 접속까지는 성공, 포트에서 TCP 연결 실패, 방화벽 문제
	- TCP 연결 타임아웃
	- TCP 소켓 연결 타임아웃
5. [[연결 종료]]
	- 정상 종료
		- `Socket.close()` 호출시 `FIN` 패킷을 전송한다.
		- `FIN` 패킷을 받으면 마찬가지로 `FIN` 을 보내야 한다.
		-  InputStream 구현체 별 EOF 반환값
	- 강제 종료
		- `Socket.close()` 호출시 `FIN` 패킷을 전송한다.
		- `FIN` 패킷을 받고 데이터 전송(PUSH)할 경우 [[RST]] 응답을 받게 됨.
	- 정리
## 디자인 패턴
1. [[커맨드 패턴]]
2. [[Null Object 패턴]]
## HTTP
1. [[HTTP 메시지 구조]]
	- GET에서 메시지 바디는 지원되지 않는 곳이 있어서 권장되지 않음
2. [[HTTP 서버]] 구현
	- HTTP 응답 전문
	- HTTP 요청 전문 헤더 해석
3. [[URL인코딩]]
	- URL은 ASCII인 이유
	- 한글 같은 유니코드를 URL인코딩하는 방법 
		- 퍼센트 인코딩을 어떤 상황에 사용하는지 사용처
4. [[WAS의 역사]]
## 리플렉션
1. [[리플렉션이 필요한 이유]]
2.  [[리플렉션에 관하여]]
	- 리플렉션의 정의
	- 리플렉션으로 얻을 수 있는 정보들
3. 리플렉션에 [[Class 메타데이터 조회]]
	- Class 메타데이터 조회방법 3가지
	- Class 메타데이터 기본정보 조회
4. [[메서드 탐색과 동적 호출]]
	- 메서드 메타데이터 조회 방법
		- public 메서드 전체 조회, 상속 받은 메서드 포함
		- private 포함 모든 선언된 메서드 조회, 상속 받은 메서드 미포함
5. [[리플렉션 동적 바인딩]]
	- `method.invoke(obj, args[])`
## 애노테이션
1. [[애노테이션에 관하여]]
	- 애노테이션의 의미 : 주석
	- 리플렉션 기능을 통해 애노테이션이라는 주석을 읽음
	- 애노테이션 정의 규칙
2. [[메타 애노테이션]]
	- 애노테이션에 애노테이션
	- @Retention
	- @Inherited
	- @Target
	- @Document
3. [[애노테이션과 상속]] : @Inherited 애노테이션에 대해
	- 애노테이션이 다른 인터페이스나 애노테이션을 상속 할 수 없는 이유
	- @Inherited 사용법
		- 인터페이스의 구현체에는 적용 안 됨.
		- 부모 클래스에 적용하면 자식 클래스도 적용 됨.
		- 클래스에만 @Inherited가 적용되는 이유
4. [[자바 기본 애노테이션]]
	- @Override : 메서드 재정의가 잘 되었는지
	- @Deprecated : 사용하지 말 것을 경고
	- @SuppressWarnings : 다 알고있으니 경고 무시
