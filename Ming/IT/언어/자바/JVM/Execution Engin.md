...
2. **실행과정** : 반복되는 코드를 발견하여 바이트 코드 전체를 컴파일하여 Native Code로 변경하고 이후에는 해당 메서드를 더이상 인터프리팅 하지 않고 캐싱해두었다가 네이티브 코드로 직접 실행하는 방식
...
#### 가비지 콜렉터
1. 더이상 참조되지 않는 메모리 객체를 모아 제거하는 역할 수행
2. 일반적으로 자동으로 실행되지만 수동으로 실행되기 위해 System.gc()를 사용할 수 있음