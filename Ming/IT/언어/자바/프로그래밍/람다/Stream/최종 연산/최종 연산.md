...
| reduce           | 누적 함수를 사용해 모든 요소를 단일 결과로 합침           | `int sum = stream.reduce(0, Integer::sum);`                          |
| min / max        | 최솟값, 최댓값을 Optional로 반환                | `stream.min(Integer::compareTo)`<br>`stream.max(Integer::compareTo)` |
...
System.out.println("짝수 리스트: " + Arrays.toString(arr));  
System.out.println();  
...
System.out.println("10. findAny - 아무 요소나 하나 찾기(멀티스레드에서 좋음)");  
